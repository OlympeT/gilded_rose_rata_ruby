...
describe('#update_quality') do
  ...
  context('with a single') do
    ...
    context 'normal item' do
      ...
      Invariant { expect(item.sell_in).to equal(initial_sell_in - 1) }

      context 'before sell date' do
        Then { expect(item.quality).to equal(initial_quality - 1) }
      end

      context 'on sell date' do
        Given(:initial_sell_in) { 0 }
        Then { expect(item.quality).to equal(initial_quality - 2) }
      end
      ...
      context 'of zero quality' do
        ...
        Then { expect(item.quality).to equal(0) }
      end
    end
    ...
    context 'Aged Brie' do
      ...
      context 'before sell date' do
        ...
        context 'with max quality' do
          ...
          Then { expect(item.quality).to equal(initial_quality) }
        end
      end
      ...
      context 'on sell date' do
        ...
        context 'near max quality' do
          ...
          Then { expect(item.quality).to equal(50) }
        end
    ...
    context 'Sulfuras' do
      ...
      Invariant { expect(item.sell_in).to equal(initial_sell_in) }
    ...
    context 'Backstage pass' do
      ...
      context 'very close to sell date (upper bound)' do
        Given(:initial_sell_in) { 5 }
        Then { expect(item.quality).to equal(initial_quality + 3) }
    ...
    context 'conjured item' do
      ...
      context 'on sell date' do
        Given(:initial_sell_in) { 0 }
        Then { expect(item.quality).to equal(initial_quality - 4) }
  ...
  context 'with several objects' do
    ...
    Then { expect(items[0].quality).to equal(9) }
    Then { expect(items[0].sell_in).to equal(4) }

    Then { expect(items[1].quality).to equal(11) }
    Then { expect(items[1].sell_in).to equal(2) }
  end
end